...
// ✅ Updated allowedLanguagesForNonPro to include Java and C++
const allowedLanguagesForNonPro = new Set([
  "javascript",
  "python",
  "ruby",
  "java",    // newly added
...
function LanguageSelector({ hasAccess }: { hasAccess: boolean }) {
  ...
  const handleLanguageSelect = (langId: string) => {
    if (!hasAccess && !allowedLanguagesForNonPro.has(langId)) return;
    setLanguage(langId);
    setIsOpen(false);
  };
    ...
    <div className="relative" ref={dropdownRef}>
      <motion.button
        ...
        className={`group relative flex items-center gap-3 px-4 py-2.5 bg-[#1e1e2e]/80 
          ...
          ${!hasAccess && !allowedLanguagesForNonPro.has(language) ? "opacity-50 cursor-not-allowed" : ""}`}
      >
        <div
          ...
          >
            ...
            <div className="max-h-[280px] overflow-y-auto overflow-x-hidden">
              {Object.values(LANGUAGE_CONFIG).map((lang, index) => {
                const isLocked = !hasAccess && !allowedLanguagesForNonPro.has(lang.id);
